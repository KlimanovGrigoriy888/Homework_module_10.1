...
def filter_by_currency(list_transactions: list[dict], currency: str) -> Iterable[dict]:
    ...
    return (x for x in list_transactions if x["operationAmount"]["currency"]["code"] == currency)
...
def transaction_descriptions(list_transactions: list[dict]) -> Iterable:
    ...
    return (x["description"] for x in list_transactions)
...
def card_number_generator(start_number: int, stop_number: int) -> int:
    ...
    for gen_number in range(start_number, stop_number+1):
        if gen_number <= 9999999999999999:
            gen_number_str = f"{gen_number:016d}"
            result_number_masks = f"{gen_number_str[:4]} {gen_number_str[4:8]} {gen_number_str[8:12]} {gen_number_str[12:16]}"
